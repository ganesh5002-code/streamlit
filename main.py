...
def complete_answer(question: str, max_rounds: int = 2) -> str:
    base_prompt("Answer clearly in numbered points. Don't cut sentences. Finish each point fully.\n\n" f"Question: {question}")
    ans = generate_response(base_prompt, temperature=0.3, max_tokens=1024)
    
    rounds = 0
    while rounds < max_rounds and looks_incomplete(ans):
        cont_prompt = ("Continue exactly from where you stopped. Do not repeat earlier text. Finish the incomplete point and complete the answer.\n\n"f"Question: {question}\n\n"f"Answer so far:\n{ans}\n\nContinue:")
...
def main():
    ...
    st.write("Welcome! Ask me about subjects and i'll give you answers.")
